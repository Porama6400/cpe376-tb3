...
class Turtlebot3Controller(Node):
    ...
    def __init__(self):
        ...
        self.pid_angular.output_cap = 0.5
    ...
    def timerCallback(self):
        ...
        laser_range = self.valueLaserRanges[0]
        if laser_range == 0:
            return
        print("position", self.valuePosition)
        print("range", laser_range)
        speed_linear = self.pid_linear.tick(laser_range - 0.3)
        speed_angular = self.pid_angular.tick(angle_calculate_delta(self.valueRotation, 0.0))
        print("speed", speed_linear, speed_angular)
        self.publishVelocityCommand(speed_linear, speed_angular)
