...
class WaypointController(object):
    def __init__(self):
        ...
        self.waypoint_distance_angle_pair: list[Vector] = []
    ...
    def go_to(self, distance_mm: int):
        distance = distance_mm / 100
        self.waypoint_distance_angle_pair.append(Vector(distance, self.builder_current_angle))

    def calculate_plan(self):
        for entry in self.waypoint_distance_angle_pair:
            distance = entry.x
            angle = entry.y + self.offset_angle
            vec = Vector(math.cos(angle) * distance, math.sin(angle) * distance)
